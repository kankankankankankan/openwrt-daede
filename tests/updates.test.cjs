...
test('legacy migration state survives the package probe', async () => {
  assert.deepEqual(await load('1.0.0-r1\t\tmigration-required\n').probePkg('dae'), { installed: '1.0.0-r1', latest: '', state: 'migration-required' });
});
